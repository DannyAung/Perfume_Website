            ...
            <div class="col-md-9">
                    <div class="container py-5">
                        <div class="row row-cols-1 row-cols-md-4 g-4">
                            ...
                            $popular_query = "SELECT *, (SELECT AVG(rating) FROM reviews WHERE product_id = products.product_id) AS avg_rating FROM products WHERE subcategory = 'popular'"; 
                            if ($category !== 'All') {
                                $popular_query .= " AND category = '$category'";
                            }
                            ...
                            if (isset($_GET['min_price']) && isset($_GET['max_price']) && is_numeric($_GET['min_price']) && is_numeric($_GET['max_price'])) {
                                ...
                                $popular_query .= " AND price BETWEEN $min_price AND $max_price";
                            }

                            // Apply Discount Filter
                            if (isset($_GET['discount']) && is_numeric($_GET['discount'])) {
                                $discount = intval($_GET['discount']);
                                $popular_query .= " AND discount_percentage >= $discount";
                            }

                            // Apply Stock Filter
                            if (isset($_GET['in_stock'])) {
                                $popular_query .= " AND stock_quantity > 0";
                            }

                            // Execute the query
                            $result = mysqli_query($conn, $popular_query);
                                ...
                                <div class="col">
                                    <div class="card h-100 text-center shadow-sm border-0 rounded product-card">
                                        ...
                                        <div class="rating mb-2">
                                            <?php $rating = (float)($popular_product['avg_rating'] ?? 0); ?>
